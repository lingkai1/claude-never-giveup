...
IDLE 时的注入决策：首次直接注入；已注入过又见 IDLE 时，监工不无脑连喊——先用三路信号确认上次**真的落地**了（90 秒 peek 窗口抓 BUSY → 屏幕找 `> 继续` 消息行 → 查该会话的 transcript 文件），确认则继续注入；三路全失败才计一次失败，连续 3 次熔断。注入后若观察到非 IDLE 状态（消息可能还在输入队列里排队），宽限 2 轮复查、不注入也不计失败，先等它消化。委派型会话（主提示符长期空闲、活儿在后台 agent）因此也不会被冤枉熔断。
...
- 读屏与注入之间有毫秒级窗口；注入落点若恰逢弹窗，文本会作为待输入排队，风险低，宽限机制 + 熔断兜底。
