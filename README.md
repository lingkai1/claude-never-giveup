...
/usr/bin/python3 claude_never_giveup.py setup   # TUI：添加监控（自动扫描在跑的会话）、启停 daemon
# 或纯命令行：
mkdir -p ~/.claude-never-giveup/monitors
...
dry_run=1
EOF
/usr/bin/python3 claude_never_giveup.py start   # 前台调试用 daemon 子命令
```
...
`claude_never_giveup.py setup` 打开交互菜单，常用操作不用手改文件：
...
- **监控列表**：每 5 秒重绘状态表（名称 / enabled / 状态 / 上次检查 / 上次注入 / 失败计数 + daemon 运行状态），`q`+回车 返回。
- **添加监控**：自动扫描在跑的 `claude -n` 命名会话列出供选（也支持手动输入会话名），再依次询问 注入词[继续] → 检测间隔[300] → dry_run[n]；重名先确认再覆盖。**没扫到会话时选「手动输入会话名」，填 iTerm2 标题里的会话名即可**（非 `-n` 启动的会话走这条路）。
- **编辑 / 删除**：选中监控后逐项重问，回车 = 保留当前值（session_name / 注入词 / 间隔 / enabled / dry_run），最后确认是否删除。
- **启停 daemon**：显示 daemon 运行状态，可启动 / 停止，也可安装 / 卸载 launchd 常驻。
- **日志尾部**：显示最近 30 行日志。

监控的增删改在 daemon 下一个 tick（15 秒内）自动生效，**无需重启**。
...
- 设计文档：`docs/specs/2026-09-06-claude-never-giveup-design.md`（含 §14 Python 修订）；移植计划：`docs/plans/2026-09-07-python-port.md`。
